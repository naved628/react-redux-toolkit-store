import React, { useEffect, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { add } from "../store/cartSlice";
import { STATUSES, fetchProducts, fetchProducts2 } from "../store/productSlice";
import { addToWish } from "../store/wishListCartSlice";

const Products = () => {
  // const [products, setProducts]= useState([]);
  const dispatch = useDispatch();
  const { data: products, status } = useSelector((state) => state.product);
  const [allProducts, setAllProducts] = useState([]);
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [searchText, setSearchText] = useState("");

  // console.log(allProducts, "data");
  // console.log(filteredProducts, "filteredProducts");
// console.log(products,'data comming');
  useEffect(() => {
    // dispatch(fetchProducts());
    dispatch(fetchProducts2());
    setAllProducts(products);
    console.log(products,'hghfhc');
    setFilteredProducts(products);
    // const fetchProducts = async()=>{
    //     const res = await fetch('https://fakestoreapi.com/products');
    //     const data= await res.json();
    //     setProducts(data);
    // }
    // fetchProducts();
  }, [products?.length]);
  const handleToAddCart = (product) => {
    dispatch(add(product));
  };

  const handleToAddWish = (product) => {
    dispatch(addToWish(product));
  };
  const filterData = (searchText, allProducts) => {
    console.log(searchText,'text result');
    // console.log(allProducts, "allProducts");
    const searchResults = allProducts.filter((product) => product.title.toLowerCase().includes(searchText || ''));
    console.log(searchResults, "search results");
    return searchResults;
  };

  if (status === STATUSES.LOADING) {
    return <h2>Loading......</h2>;
  }

  if (status === STATUSES.ERROR) {
    return <h2>Something went wrong...</h2>;
  }

  return (
    <>
      <div className="search-container">
        <input
          type="text"
          placeholder="Search products"
          value={searchText}
          className="search-input"
          onChange={(e) => setSearchText(e.target.value)}
        />
        <button className="search-btn"
          // disabled={!searchText}
          onClick={() => {
            const data = filterData(searchText, allProducts);
            setFilteredProducts(data);
          }}
        >
          Search
        </button>
      </div>
      <div className="productsWrapper">
        {filteredProducts.length <= 0 ? (
          <h3>No results found</h3>
        ) : (
          filteredProducts.map((product) => (
            <div className="card" key={product.id}>
              <div className="image-container">
                <img src={product?.images[0]} alt={product?.title} />
              </div>
              <div className="title-price-container">
                <div className="title">{product?.title}</div>
                <div className="price">${product?.price}</div>
              </div>
              <div className="button-container">
                <button
                  className="btn"
                  onClick={() => handleToAddCart(product)}
                >
                  Add to cart
                </button>
                <button
                  className="btn"
                  onClick={() => handleToAddWish(product)}
                >
                  wish list
                </button>
              </div>
            </div>
          ))
        )}
      </div>
    </>
  );
};

export default Products;
